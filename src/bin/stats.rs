...
fn main() {
    ...
    for line in reader.lines() {
        if let Ok(l) = line {
            if let Ok(row) = serde_json::from_str::<AmpRow>(&l) {
                ...
                entry.turns += 1;
                entry.input_total_sum += row.input_total;
    ...
    for (_, stats) in stats_map {
        ...
        let cache_hit_rate = if stats.cache_read_total_sum + stats.input_total_sum == 0 {
            0.0
        } else {
            stats.cache_read_total_sum as f64 / (stats.cache_read_total_sum as f64 + stats.input_total_sum as f64)
        };
